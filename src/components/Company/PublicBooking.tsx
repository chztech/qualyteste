...
import { formatDate, formatDateWithWeekday, isDateTimePast, isDatePast } from '../../utils/dateUtils';
...
interface AvailableSlot {
  ...
  time: string;
  service: string;
  provider: Provider | null; // agora pode ser null
  duration: number;
...
export default function PublicBooking({
  ...
  const getAvailableSlotsForCompany = (): AvailableSlot[] => {
    ...
    // slots = appointments da empresa sem employeeId (vaga aberta) e no futuro
    ...
    const availableSlots = companySlots.map(apt => {
      ...
      return {
        date: apt.date,
        time: apt.startTime,
        service: (apt as any).service ?? apt.serviceName ?? "",
        provider,
  ...
  const getFilteredSlots = (): AvailableSlot[] => {
    ...
    if (selectedShift && selectedShift !== '') {
      filteredSlots = filteredSlots.filter(slot => getShiftFromTime(slot.time) === selectedShift);
    }

    filteredSlots = filteredSlots.filter(slot => !isDateTimePast(slot.date, slot.time));
  ...
  useEffect(() => {
    // logs simples
    // console.log({companyToken, company, appointments: appointments.length, slots: availableSlots.length});
        ...
        <div className="bg-white rounded-lg shadow-sm p-4 sm:p-6">
                  ...
                  >
                    <div className="flex items-center space-x-2 mb-3">
                      ...
                      <span className="font-medium text-gray-900 text-sm sm:text-base">
                        {formatDate(dateInfo.date, { weekday: 'short', day: '2-digit', month: '2-digit' })}
                      </span>
                    </div>
          ...
          {step === 3 && selectedDate && (
            <div className="space-y-6">
              ...
              {(() => {
                const dateInfo = getAvailableDates().find(d => d.date === selectedDate);
